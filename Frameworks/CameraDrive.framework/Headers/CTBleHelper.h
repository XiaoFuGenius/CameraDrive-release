...
extern NSNotificationName const CT_iPhone_BleUpdate;        // 手机蓝牙的 启停状态 变化通知
extern NSNotificationName const CT_Device_ScanUpdate;       // 手机蓝牙 已扫描到的设备列表 更新通知
extern NSNotificationName const CT_Device_BleUpdate;        // 设备与手机的 蓝牙连接状态 变化通知
extern NSNotificationName const CT_Device_BatteryUpdate;    // 设备 电池状态（充电，电量）变化通知

typedef NS_ENUM(NSInteger, CTBleResponseCode) {
    CTBleResponseOK = 0,        // base code - succeed
    CTBleResponseError = -1,    // base code - failed
};
typedef void(^CTResponseHandler)(CTBleResponseCode code, NSString *msg);
...
/**
 ...
 @param response 请求回调
 */
+ (void)Shutdown:(CTResponseHandler)response;
...
/**
 ...
 注：应用时建议设定超时处理
 */
+ (void)Bind:(CTResponseHandler)response;
 ...
 */
+ (void)NetworkStatus:(void(^)(CTBleResponseCode code, int type,
                                NSString *ssid, NSString *password,
                                NSString *ip))response;

/**
 请求 设备 启动热点模式
 @param response 请求回调
 */
+ (void)AP:(void(^)(CTBleResponseCode code, NSString *ssid, NSString *password))response;
...
+ (void)STA_VerifiedSSID:(NSString*)ssid
                Response:(void(^)(CTBleResponseCode code, int status, NSString *msg))response;
...
+ (void)STA:(NSString *)ssid Password:(NSString *)password
   Response:(void(^)(CTBleResponseCode code, int wifiStatus, NSString *ip))response;
...
/**
 获取 设备 的 充电状态&电量 信息
 */
+ (void)Battery:(void(^)(CTBleResponseCode code, BOOL isCharge, int battery))response;

/**
 获取 设备 的 MAC 信息
 */
+ (void)MAC:(void(^)(CTBleResponseCode code, NSString *mac))response;

/**
 获取 设备 的 蓝牙&核心 固件版本信息
 注：内部会启动主控芯片
 */
+ (void)Version:(void(^)(CTBleResponseCode code, NSString *ble, NSString *core, long bleValue, long coreValue))response;
 ...
 */
+ (void)CalibrateStatusCheck:(void(^)(CTBleResponseCode code, int status, NSString *msg))response;
...
/**
 ...
 注2：最低版本要求，Ble 2.1.0 & Core 3.1.0
 */
+ (void)CalibrateCommand:(int)command Response:(void(^)(CTBleResponseCode code, int status, NSString *msg))response;
...
/**
 ...
 注2：最低版本要求，Ble 2.1.0 & Core 3.1.0
 */
+ (void)CalibrateRestartNVDS:(void(^)(CTBleResponseCode code, int status, NSString *msg))response;

/**
 无线&有线 模式 设置
 @param command 设置命令，
 -1：混合模式（旧设备），-2：无线模式（旧设备），-3：有线模式（旧设备）
  1：混合模式（新设备）， 2：无线模式（新设备）， 3：有线模式（新设备）
 @param response 请求回调
 注：最低版本要求，Ble 2.1.0 & Core 3.1.0
 */
+ (void)SetWiredModeCommand:(int)command Response:(void(^)(CTBleResponseCode code, NSString *msg))response;
