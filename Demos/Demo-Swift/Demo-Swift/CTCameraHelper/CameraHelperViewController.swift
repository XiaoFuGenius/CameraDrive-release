//
//  CameraHelperViewController.swift
//  Demo-Swift
//
...
// Privacy Status 状态
enum XFUserAuthorizationStatus: Int {
    case ErrorParam = 0
    case NotSupport  = 1
...
class CameraHelperViewController: UIViewController {

    open var ip: String = ""
    open var handler_log = { (log: NSString) in }

    let backgroundTaskId = UIBackgroundTaskIdentifier.init(rawValue: 912)  // 后台任务标识符；针对于图像采集界面的前后台操作，不是必须的；
    var appIsActived: Bool = false
    var cameraIsActived: Bool = false
    var cameraIsRestart: Bool = false
    ...
    @objc func captureBtnClick(sender: UIButton) {
        ...
        CTCameraHelper.shared()?.captureRgbFilePath(rgbName, plFilePath: plName, handler: { (status, description) in
    ...
    @objc func layerBtnClick(sender: UIButton) {
        ...
        CTCameraHelper.shared()!.ledMode = self.ledMode
    }
    ...
    func prepareForCameraStart() {
        CTCameraHelper.shared()!.renderingBitrate = 800
        CTCameraHelper.shared()!.ledMode = 0
        CTCameraHelper.shared()!.isRetroflexion = true
        CTCameraHelper.shared()!.ip = self.ip
        CTCameraHelper.shared()!.port = 1000
        CTCameraHelper.shared()!.loadBearerView(self.cameraView) { (status, description) in
            NSLog("%@", description!)
        }

        self.appIsActived = true
        self.cameraIsActived = false
        self.cameraIsRestart = false

        NotificationCenter.default.addObserver(self, selector: #selector(cameraActiveStatusUpdate(noti:)),
    ...
    func resetCameraAfterCtrDealloc() {
        CTCameraHelper.shared()!.unloadBearerView { (status, description) in
            NSLog("%@", description!)
        }
    }

    func startCamera() {
        self.layerBtn.isSelected = false
        self.captureBtn.isSelected = false

        weak var weakSelf = self
        CTCameraHelper.shared()!.start { (status, description) in
            NSLog("%@", description!)
            DispatchQueue.main.async {
    ...
    func stopCamera() {
        ...
        //weak var weakSelf = self
        CTCameraHelper.shared()!.stop { (status, description) in
            NSLog("%@", description!);
        }
    }
    ...
    func cameraStatusUpdate(isOK: Bool) {
        ...
        DispatchQueue.main.async {
            ...
            NSLog("摄像头 当前已启动.")
            CTCameraHelper.shared()?.ledMode = (weakSelf?.ledMode)!
            weakSelf?.layerBtn.isSelected = true
            weakSelf?.captureBtn.isSelected = true
        }
    }

    @objc func cameraActiveStatusUpdate(noti: Notification) {
        if noti.name == UIApplication.didBecomeActiveNotification {
            NSLog("应用处于活动状态.")
        } else {
            NSLog("应用处于非活动状态.")
        }
        ...
        weak var weakSelf = self
        DispatchQueue.main.async {
            weakSelf?.appIsActived = noti.name == UIApplication.didBecomeActiveNotification
            ...
            if !(weakSelf?.appIsActived)! {
                ...
                weakSelf?.perform(#selector(weakSelf!.cameraWillClose), with: nil, afterDelay: 3.0)
                return
            }
            ...
            weakSelf?.layerBtn.isSelected = false
            weakSelf?.captureBtn.isSelected = false
            weakSelf?.perform(#selector(weakSelf!.cameraWillRestart), with: nil, afterDelay: 1.5)
        }
    }

    @objc func cameraWillRestart() {
        if self.cameraIsActived || self.cameraIsRestart {
            NSLog("<准备重新打开摄像头>已触发，状态重复，取消<准备重新打开摄像头>.")
            self.layerBtn.isSelected = true
            self.captureBtn.isSelected = true
            return
        }

        NSLog("<准备重新打开摄像头>已触发，正在重新打开摄像头.")
        self.cameraIsRestart = true
        self.startCamera()
    }

    @objc func cameraWillClose() {
        if !self.cameraIsActived && !self.cameraIsRestart {
