...
- (void)viewDidLoad {
    ...
    [self.notiCenter addObserver:self selector:@selector(cameraBackgroundStatusUpdate:)
...
- (void)cameraBackgroundStatusUpdate:(NSNotification *)noti
{
    XFWeakSelf(weakSelf);
...
- (void)cameraStatusUpdate:(BOOL)isOK
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (!isOK) {
            if (weakSelf.camera.isBlueStripConfirmed) {
                [weakSelf showAlertViewMsg:@"当前检测到蓝条，摄像头已关闭，可尝试重启或者退出当前控制器."];
            } else {
                [weakSelf showAlertViewMsg:@"摄像头发生错误，当前已关闭，可尝试重启或者退出当前控制器."];
            }
